...
test('renders children', (t) => {
  const MyComponent = ({ foo, children }) => {
    return React.createElement('div', { className: 'parent' }, 'hello', children)
  }

  const c2 = () => {
    return React.createElement('div', { className: 'wow' }, 'hello from c2')
  }
...
test('initial state rehydration', (t) => {
  const component = `
    function MyComponent ({ foo, _state }) {
      return React.createElement('p', { 'data-state': _state }, 'the value of foo is "' + foo + '"')
    }
  `
  ...
  return reshape({ plugins: [components({ 'my-component': MyComponent })] })
    ...
    .then((res) => {
      const dom = new JSDOM(`
        ...
        <script>
          // require react
          ${fs.readFileSync(path.join(__dirname, '../node_modules/react/dist/react.min.js'))}
          // require reshapeReact
          ${fs.readFileSync(path.join(__dirname, '../lib/browser.js'))}
          // our custom component
          ${component}
          // implementation
          var el = document.querySelector('p')
          window.out = reshapeReact.hydrateInitialState(el.getAttribute('data-state'), {
            'my-component': MyComponent
          })
        </script>
      `, { runScripts: 'dangerously' })
      t.is(renderToStaticMarkup(dom.window.out), '<p>the value of foo is &quot;bar&quot;</p>')
    })
})
...
test('browser encode and decode', (t) => {
  ...
  const dom = new JSDOM(`
    <script>
      // require react, because its referenced in the browser lib
      ${fs.readFileSync(path.join(__dirname, '../node_modules/react/dist/react.min.js'))}
      // require browser lib
      ${fs.readFileSync(path.join(__dirname, '../lib/browser.js'))}
      // implementation
      var encoded = reshapeReact.encode(${data})
      window.decoded = reshapeReact.decode(encoded)
    </script>
  `, { runScripts: 'dangerously' })
